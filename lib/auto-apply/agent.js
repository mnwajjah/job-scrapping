...
async function processJob(job) {
  ...
  // Buat folder html log jika belum ada
  const htmlLogDir = path.join(LOG_DIR, "html");
  fs.mkdirSync(htmlLogDir, { recursive: true });
  ...
  while (stepCount < maxSteps) {
    ...
    // Log html ke file untuk diagnosa
    const safeJobTitle = (job.title || "unknown").replace(/[^a-zA-Z0-9_-]/g, "_").slice(0, 30);
    const htmlLogFile = path.join(htmlLogDir, `${safeJobTitle}_step${stepCount}.html`);
    fs.writeFileSync(htmlLogFile, html, "utf-8");

    // Cek jika HTML identik dengan step sebelumnya (stuck)
    if (lastHtml && lastHtml === html) {
      log("⚠️  Halaman tidak berubah setelah aksi sebelumnya. Menghentikan proses untuk mencegah loop tak terbatas.");
      return { success: false, reason: "Halaman macet / form submit gagal berubah" };
    }
    lastHtml = html;
