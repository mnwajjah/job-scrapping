...
async function launch(opts = {}) {
  ...
  browser = await chromium.launch({
    headless: opts.headless ?? false, // Default: tampilkan browser
    slowMo: opts.slowMo ?? 300,       // Delay antar aksi (ms) agar terlihat
  ...
  // Block resource-heavy stuff yang tidak perlu
...
async function clickElement(selector) {
  try {
    ...
    // Fallback: coba klik by text jika selector gagal
    try {
      // Coba cari berdasarkan teks tombol yang umum
      const applyTexts = ["Apply", "Apply Now", "Submit", "Submit Application", "Next"];
      for (const text of applyTexts) {
        const btn = page.getByRole("button", { name: text }).or(
