...
async function launch(opts = {}) {
  ...
  try {
    ...
    if (contexts.length > 0) {
      context = contexts[0];
      // Selalu buka tab baru agar tidak mengganggu/terganggu oleh aktivitas tab aktif user
...
async function getCleanHTML() {
  return page.evaluate(() => {
    ...
    // Hapus elemen yang tidak relevan (clutter & widgets)
    const removeSelectors = [
      "script", "style", "svg", "img", "video", "audio", "iframe",
      "noscript", "link[rel=stylesheet]", "meta", "header", "footer",
      "[role=banner]", "[role=navigation]", "nav",
      "[class*=cookie]", "[class*=popup]", "[class*=modal]", "[class*=dialog]",
      "[class*=advertisement]", "[class*=promo]", "[class*=banner]",
      "[class*=messaging]", "[class*=chat]", "[id*=messaging]", "[id*=chat]",
      "[class*=sidebar]", "[id*=sidebar]", "[class*=aside]", "[id*=aside]",
      "[class*=menu]", "[class*=dropdown]", "[class*=notification]",
      "[class*=share]", "[class*=report]", "[class*=premium]", "[class*=footer]",
...
async function clickElement(selector) {
  try {
    ...
    // 2. Fallback: coba trigger click event langsung via JS jika element ada di DOM
    try {
      const el = page.locator(selector).first();
      if (await el.count() > 0) {
        await el.dispatchEvent("click", { timeout: 2000 });
        await page.waitForTimeout(2000);
    ...
    // 3. Fallback: cari tombol berdasarkan teks umum (English & Indonesian)
    try {
      const applyTexts = [
        "Apply", "Apply Now", "Lamar", "Lamar Sekarang", 
        "Submit", "Kirim", "Kirim lamaran",
        "Next", "Berikutnya", "Lanjut"
      ];
      for (const text of applyTexts) {
        ...
        if (await btn.count() > 0) {
          try {
            ...
            await btn.dispatchEvent("click", { timeout: 2000 });
            await page.waitForTimeout(2000);
            return true;
          }
        }
